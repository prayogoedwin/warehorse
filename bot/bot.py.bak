...
class DatabaseManager:
    ...
    def get_mssql_connection():
        """Get MSSQL connection using pymssql"""
        import pymssql
...
def main():
    ...
    async def run_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
        if context.args and context.args[0].lower() == "sync":
            await run_sync(update, context)
        else:
            await update.message.reply_text("Format: /run Sync")
    
    async def stop_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
        if context.args and context.args[0].lower() == "sync":
            await stop_sync(update, context)
        else:
            await stop_bot(update, context)
    
    application.add_handler(CommandHandler("run", run_router))
    ...
    async def post_init(application: Application):
        await application.bot.set_my_commands([
            ...
            BotCommand("run", "Jalankan sync penuh"),
